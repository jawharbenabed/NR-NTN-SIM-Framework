...
int main(int argc, char* argv[])
{
    ...
    ueNodes.Get(0)->GetObject<GeocentricConstantPositionMobilityModel>()
        ->SetGeographicPosition(Vector(48.85, 2.35, 0.0));
    ...
    DynamicCast<NrUeNetDevice>(ueNetDev.Get(0))->GetBwpManager()->SetOutputLink(0, 1);
    ...
    uint32_t lambdaDl  = g_lambdaDl;
    // ── UL saturation — utilise g_lambdaUl (configurable par CMD) ──────────
    // 20000 pkt/s = 200 Mbps offert → caractérisation canal UL complète
    // Référence : TR 38.821 §6.3.2 UL throughput characterization
    uint32_t lambdaUl  = g_lambdaUl;

    ApplicationContainer serverApps, clientApps;

    // DL server sur UE
    UdpServerHelper dlSink(PORT_DL_DATA);
    serverApps.Add(dlSink.Install(ueNodes.Get(0)));

    // DL client sur RemoteHost
    ...
    dlClient.SetAttribute("Remote",
        AddressValue(addressUtils::ConvertToSocketAddress(
            ueIpIface.GetAddress(0), PORT_DL_DATA)));
    clientApps.Add(dlClient.Install(remoteHost));

    // DL bearer : NGBR_LOW_LAT_EMBB sur BWP0 — filtre DL uniquement
    NrQosFlow dlFlow(NrQosFlow::NGBR_LOW_LAT_EMBB);
    Ptr<NrQosRule> dlRule = Create<NrQosRule>();
    NrQosRule::PacketFilter dlpf;
    dlpf.localPortStart  = PORT_DL_DATA;  // port local UE (réception DL)
    dlpf.localPortEnd    = PORT_DL_DATA;
    dlpf.direction       = NrQosRule::DOWNLINK;
    dlRule->Add(dlpf);
    nrHelper->ActivateDedicatedQosFlow(ueNetDev.Get(0), dlFlow, dlRule);

    // ------------------------------------------------------------------------
    // Applications UL : UE -> RemoteHost
    // ------------------------------------------------------------------------

    // UL server sur RemoteHost
    UdpServerHelper ulSink(PORT_UL_DATA);
    ...
    // UL: QoS flow dédié GBR_CONV_VOICE mappé sur BWP1 (API native 5G-LENA v4.2)
    nrHelper->SetUeBwpManagerAlgorithmAttribute("GBR_CONV_VOICE", UintegerValue(1));
    nrHelper->SetGnbBwpManagerAlgorithmAttribute("GBR_CONV_VOICE", UintegerValue(1));
    NrQosFlow ulFlow(NrQosFlow::GBR_CONV_VOICE);
    Ptr<NrQosRule> ulRule = Create<NrQosRule>();
    NrQosRule::PacketFilter ulFilter;
    ulFilter.remotePortStart = PORT_UL_DATA;  // destination RemoteHost:1244
    ulFilter.remotePortEnd   = PORT_UL_DATA;
    ulFilter.direction       = NrQosRule::UPLINK;
    ulRule->Add(ulFilter);
    nrHelper->ActivateDedicatedQosFlow(ueNetDev.Get(0), ulFlow, ulRule);
    ...
    g_monitor->CheckForLostPackets();
    auto stats = g_monitor->GetFlowStats();
    std::cout << "\n=== FINAL RESULTS (optionD NTN) ===\n";
    std::cout << "satAlt=" << g_satAltKm << "km  txPower=" << g_txPowerDbm << "dBm\n";
    for (const auto& kv : stats) {
